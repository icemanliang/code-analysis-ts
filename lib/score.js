// 默认的代码评分逻辑
exports.scoreDeal = function (coderObj){
    const { apiMap, noUseMap,  noParseFiles, browserApiMap } = coderObj;
    let score = 100;            // 初始分数
    let message =[];            // 代码建议
    ...
    if(noParseFiles.length >0){
        score = score - 3*noParseFiles.length;
        let tempMessage ='';
        tempMessage = noParseFiles.length + '个文件解析AST或执行分析异常，建议修复';
        message.push(tempMessage);
    }
