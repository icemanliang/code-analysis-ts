// 默认评分插件
exports.defaultScorePlugin = function (codeObj){
    const { apiMap, noUseMap,  parseErrorInfos, browserApiMap } = codeObj;
    let score = 100;            // 初始分数
    let message =[];            // 代码建议
    ...
    Object.keys(browserApiMap).forEach((fitem)=>{
        Object.keys(browserApiMap[fitem]).forEach((sitem)=>{
            ...
            if(fitem ==='history'){
                score = score - 2;
                message.push(fitem+'.'+sitem+' 属于路由类操作，请使用框架提供的Router API代替');
            }
            if(fitem ==='location'){
                score = score - 2;
                message.push(fitem+'.'+sitem+' 属于路由类操作，请使用框架提供的Router API代替');
    ...
    Object.keys(noUseMap).forEach((fitem)=>{
        if(noUseMap[fitem].callNum >0 && noUseMap[fitem].callFiles.length >0){
            score = score - 2;
            let tempMessage ='';
            tempMessage = tempMessage + fitem + ' 被引入但未使用，请修复';
