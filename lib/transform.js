// apiMap数据转换处理
exports.transformApiMap = function(map, importMap, blackApis = []) {
    let result = {};
    Object.keys(importMap).forEach(element => {
      result[element] = {};
    });
          ...
          if (temp[0] == relement) {
            if (temp.length == 2) {
              if (!result[relement][temp[1]]) {
                ...
                result[relement][temp[1]].callFiles = map[melement].callFiles;
              ...
              if (!result[relement][temp[1]]) {
                ...
                result[relement][temp[1]].children[temp[2]].callFiles = map[melement].callFiles;
                if(blackApis.length>0 && blackApis.includes(melement)){   // 标记黑名单
