...
const { scoreDefaultDeal } = require(path.join(__dirname, './score'));                                        // 代码评分
...
class CodeAnalysis {
  constructor(options) {
    ...
    this.scoreFun = options.scoreFun || null;                                         // 代码评分处理函数, default才用默认处理函数
  ...
  scanVue(scanPath) {
    let entryFiles = [];
    let entryFilesPath = [];
    scanPath.forEach((item)=>{
      let tempEntry = scanFileVue(item);
      let tempPath = tempEntry.map((titem)=>{
        return titem.substring(titem.indexOf(item));
      })
      entryFiles = entryFiles.concat(tempEntry);
      entryFilesPath = entryFilesPath.concat(tempPath);
    })
    // console.log(entryFiles);
    // console.log(entryFilesPath);
    if (entryFiles.length == 0) return;
    entryFiles.forEach((element, eIndex) => {
      const mfpath = entryFilesPath[eIndex];
      try {
  ...
  analysis() {
    ...
    // 代码评分
    if(this.scoreFun){
      if(typeof(this.scoreFun) ==='function'){
        this.scoreMap = this.scoreFun(this);
      }
      if(this.scoreFun ==='default'){
        this.scoreMap = scoreDefaultDeal(this);
      }
    }else{
      this.scoreMap = null;
    }
