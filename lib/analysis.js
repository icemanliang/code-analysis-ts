...
const { scanFileTs, scanFileVue } = require(path.join(__dirname, './file'));                                  // 读写模块
...
class CodeAnalysis {
  constructor(options) {
    ...
    this._blackApis = options.blackApis || [];                                         // 需要标记的黑名单API配置        
    ...
    if(queue.length>0){
      queue.forEach((item)=>{
        Object.keys(this[item.mapName]).forEach((apiName)=>{
          if(this._blackApis.length>0 && this._blackApis.includes(apiName)){          // 标记黑名单
  ...
  _findImportItems(ast, filePath, baseLine = 0) {
    ...
    function dealImports(temp){
      ...
      if (!that.importItemMap[temp.name]) {
        ...
        that.importItemMap[temp.name].origin = temp.origin;
        that.importItemMap[temp.name].lines = [];
        that.importItemMap[temp.name].lines.push(filePath);
      } else {
        that.importItemMap[temp.name].lines.push(filePath);
      }
    }
