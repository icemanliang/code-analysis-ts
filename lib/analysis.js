...
class CodeAnalysis {
  constructor(options) {
    ...
    this._analysisPlugins = options.analysisPlugins || [];                             // 代码分析插件           
  ...
  // 链式调用检查，找出链路顶点node
  _checkPropertyAccess(node, index =0) {
    if(tsCompiler.isPropertyAccessExpression(node.parent)){   
      index++;
      return this._checkPropertyAccess(node.parent, index);
    }else{                                                                                                                          
      return {
        BaseNode :node, 
        depth: index
  ...
  // 执行分析插件
  _runAnalysisPlugins(node, depth, filepath, projectName, httpRepo, line) {
  ...
  _findImportItems(ast, filepath, baseLine = 0) {
    ...
    function dealImports(temp){
      ...
      importItems[temp.name].pos = temp.pos;
      importItems[temp.name].end = temp.end;
  ...
  _dealAST(importItems, ast, checker, filepath, projectName, httpRepo, baseLine = 0) {
    const that = this;
    const importItemNames = Object.keys(importItems);         // 从analysisTarget导入的API
    // let _noUseMap = {};
    ...
    function walk(node) {
      ...
      // new
      if(tsCompiler.isIdentifier(node) && node.escapedText && importItemNames.includes(node.escapedText)) {                                     // 命中importItem name
        const matchImportItem = importItems[node.escapedText];
        // console.log(matchImportItem);
        if(node.pos !=matchImportItem.pos && node.end !=matchImportItem.end){                                                                   // 排除importItem Node自身
          const symbol = checker.getSymbolAtLocation(node);
          // console.log(symbol);
          if(symbol && symbol.escapedName && symbol.escapedName == node.escapedText && symbol.declarations && symbol.declarations.length>0){    // 存在上下文声明
            const nodeSymbol = symbol.declarations[0];
            if(matchImportItem.pos == nodeSymbol.pos && matchImportItem.end == nodeSymbol.end){                                                 // 上下文声明与import item匹配, 符合API调用
              if(node.parent){
                const { BaseNode, depth } = this._checkPropertyAccess(node);                                                                    // 获取基础分析节点
                this._runAnalysisPlugins(BaseNode, depth, filepath, projectName, httpRepo, line);                                               // 执行分析插件                                              // 执行分析插件
              }else{
